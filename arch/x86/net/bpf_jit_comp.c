...
struct jit_emit_context {
	u8 *prog;                     /* current JIT buffer write position */
	bool dry_run;
};

static u8 *__emit_code(u8 *ptr, u32 bytes, unsigned int len)
{
	if (len == 1)
		*ptr = bytes;
	else if (len == 2)
...
static void emit_code(struct jit_emit_context *jit, u32 bytes, unsigned int len)
{
	if (jit->dry_run)
		jit->prog += len;
	else
		jit->prog = __emit_code(jit->prog, bytes, len);
}

#define EMIT(bytes, len) emit_code(jit, bytes, len)
 ...
 */
static void emit_prologue(struct jit_emit_context *jit, u8 *ip, u32 stack_depth, bool ebpf_from_cbpf,
...
static int emit_rsb_call(struct jit_emit_context *jit, void *func, void *ip)
{
	...
	ip += x86_call_depth_emit_accounting(&jit->prog, func, ip, jit->dry_run);
	return emit_patch(jit, func, ip, 0xE8);
}
...
static void emit_insn_suffix_SIB(struct jit_emit_context *jit, u32 ptr_reg, u32 val_reg, u32 index_reg, int off)
{
	if (is_imm8(off)) {
...
static void emit_ldx_index(struct jit_emit_context *jit, u32 size, u32 dst_reg, u32 src_reg, u32 index_reg, int off)
{
	switch (size) {
	case BPF_B:
...
static void emit_ldsx_index(struct jit_emit_context *jit, u32 size, u32 dst_reg, u32 src_reg, u32 index_reg, int off)
{
	switch (size) {
	case BPF_B:
...
static void emit_stx_index(struct jit_emit_context *jit, u32 size, u32 dst_reg, u32 src_reg, u32 index_reg, int off)
{
	switch (size) {
	case BPF_B:
...
static void emit_st_index(struct jit_emit_context *jit, u32 size, u32 dst_reg, u32 index_reg, int off, int imm)
{
	switch (size) {
	case BPF_B:
...
static int emit_kfunc_arena_args(struct bpf_prog *bpf_prog,
				 const struct bpf_insn *insn, struct jit_emit_context *jit)
{
...
populate_extable:
		...
		case BPF_JMP | BPF_CALL: {
			...
			if (src_reg == BPF_PSEUDO_KFUNC_CALL) {
				err = emit_kfunc_arena_args(bpf_prog, insn, jit);
				if (err < 0)
					return err;
				ip += err;
			}
 ...
 */
static void emit_arena_arg_conv(struct jit_emit_context *jit, u32 src_reg, bool nullable, u32 base_lo)
{
	if (nullable) {
		if (src_reg != BPF_REG_0)
					 ...
					 void *func_addr)
{
	...
	if (flags & BPF_TRAMP_F_INDIRECT) {
		 ...
		 * __fentry__ call.
		 */
		x86_call_depth_emit_accounting(&jit->prog, NULL, image, jit->dry_run);
	}
	EMIT1(0x55);		 /* push rbp */
	EMIT3(0x48, 0x89, 0xE5); /* mov rbp, rsp */
	if (im)
...
static int emit_bpf_dispatcher(struct jit_emit_context *jit, int a, int b, s64 *progs, u8 *image, u8 *buf)
{
	...
	__emit_code(jg_reloc - jg_bytes, jg_offset, jg_bytes);
